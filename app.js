    ...
    getProfiles: () => ({
        'lider': {
            ...
            sections: [
                ...
                { isFull: true, title: '🛒 3. Recomendações (Foco na Facção)', items: ['<strong>🏰 Estrutura de Facção (Principal):</strong> VIP Facção Premium: Mais carros, baú extra, log de baú. (✔ Mais controle, ✔ Facção mais estruturada).', '<strong>💸 Melhorar Economia Interna:</strong> Farm vaquinha / pescaria: Gera dinheiro dentro da facção. (✔ Independência financeira e menos dependência da rua).', '<strong>⚙️ Farm Ativos:</strong> Produção de farm dentro do QG. Não precisa sair para farmar. (✔ Segurança, eficiência e mais produção).', '<strong>🎁 Incentivo para Membros (Muito Forte):</strong> Compra de VIPs para sorteio: VIP PVP, VIP Ouro. (✔ Incentiva membros e aumenta engajamento).', '<strong>🎭 Identidade Visual:</strong> Uniforme, nome da facção e personalização completa do QG (Telão, portão, outdoor).'] },
        ...
        const database = {
            ...
            'ataque': {
                ...
                sections: [
                    ...
                    { title: "⚙️ Como Funciona na Prática", items: ["1️⃣ Faz ele sentir o problema (Dor).", "2️⃣ Mostra o que ele pode ganhar (Desejo).", "3️⃣ Dá um motivo para agir agora (Urgência)."] },
...
const Controller = {
    ...
    templates: {
        menu: () => `
            ...
            <style>
                .funnelStepTitle { font-size: 1.08rem !important; }
                .funnelStepDesc { font-size: 0.84rem !important; line-height: 1.4; text-wrap: balance; }
            </style>
        ...
        category: (baseData) => {
            ...
            return `
                ...
                <style>
                    ...
                    .dataList li { font-size: 0.86rem !important; line-height: 1.5; margin-bottom: 6px; white-space: nowrap; width: fit-content; position: relative; }
                    .dataList li::after { content: ""; display: inline-block; width: 40px; height: 1px; pointer-events: none; }
                    .infoCard { overflow-x: auto; scrollbar-width: thin; scrollbar-color: rgba(212,175,55,0.3) transparent; }
                    .navItem { font-size: 0.86rem !important; white-space: nowrap; }
                    ...
                    .listDivider { margin: 12px 0; border-bottom: 1px solid rgba(255,255,255,0.1); list-style: none; width: 100%; }
                </style>
                                        ...
                                        <div class="profileSubMenu" style="margin-left: 1rem; border-left: 1px solid rgba(212,175,55,0.3); margin-bottom: 10px;">
                                            ${Object.values(profiles).map(p => `
                                                <div class="navItem ${BusinessLogic.state.currentProfileId === p.id ? 'navSubItemActive' : ''}" 
                                                     style="font-size: 0.8rem !important; padding: 0.5rem 0.8rem; text-transform: none;" 
