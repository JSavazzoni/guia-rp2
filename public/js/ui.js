const UI = {
    ...
    templates: {
        ...
        category: (data) => {
            if (!data) return '<div class="overlay"></div><div style="color:#fff; padding:100px; text-align:center;">Carregando dados...</div>';
            ...
            return `
                <style>
                    ...
                    .infoCard { background: rgba(15,15,15,0.85); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.08); border-top: 1px solid rgba(212, 175, 55, 0.3); border-radius: 20px; padding: 2.5rem; display: flex; flex-direction: column; box-shadow: 0 15px 35px rgba(0,0,0,0.6); transition: 0.3s; }
                    ...
                    ${data.videoCategories ? data.videoCategories.map(cat => `
                        <div style="margin-top: 6rem; border-top: 1px solid rgba(255,255,255,0.08); padding-top: 4rem;">
                            ...
                            <div style="display:grid; grid-template-columns: repeat(2, 1fr); gap: 3rem;">
                                ${cat.videos.map(v => `
                                    <div style="background:rgba(15,15,15,0.85); border:1px solid rgba(255,255,255,0.08); padding:1.5rem; border-radius:20px; box-shadow: 0 15px 35px rgba(0,0,0,0.6);">
